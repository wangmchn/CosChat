...
//    frame.size.width = 0;
    ...
    [UIView transitionWithView:self duration:0.55 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
        ...
        if ([self.delegate respondsToSelector:@selector(matchViewDidDisapper:)]) {
            [self.delegate matchViewDidDisapper:self];
        }
    }];
}
- (void)tap:(UIGestureRecognizer *)sender{
    ...
    if (self.contentView.contentOffset.x == 0) {
        if ([self.delegate respondsToSelector:@selector(matchViewDidPressedByUser:)]) {
            [self.delegate matchViewDidPressedByUser:self];
        }
    }else{
        [self.contentView setContentOffset:CGPointMake(0, 0) animated:YES];
    }
}
- (void)setContent:(WKMatchContent *)content{
    _content = content;
    [self updateContent];
}
- (void)updateContent{
